...
function submitProduction() {
    ...
    if (existing) {
        ...
        state.products.push({
            ...
            name: prodName,
            qty: totalQty,
            costPrice: totalBatchExp / totalQty
        });
    }
...
function updateUI() {
    try {
        ...
        const dayData = (state.history && state.history[dStr]) ? state.history[dStr] : { production: [], sales: [], paidWorkers: [] };

        // Safety Fallbacks for Arrays
        const safeProd = Array.isArray(dayData.production) ? dayData.production : [];
        const safeSales = Array.isArray(dayData.sales) ? dayData.sales : [];
        const safePaid = Array.isArray(dayData.paidWorkers) ? dayData.paidWorkers : [];
...
function calculateSalaries(dayData) {
    ...
    const prodList = Array.isArray(dayData.production) ? dayData.production : [];
